...
*/
/******************************************************************************/
SpartanupPartitionHeader::SpartanupPartitionHeader(ImageHeader* imageheader, int index)
...
/******************************************************************************/
void SpartanupPartitionHeader::ReadHeader(std::ifstream& ifs)
{
    ifs.read((char*)pHTable, sizeof(SpartanupPartitionHeaderTableStructure));

    uint32_t checksum = ComputeWordChecksum(pHTable, sizeof(SpartanupPartitionHeaderTableStructure) - sizeof(uint32_t));
...
/******************************************************************************/
void SpartanupPartitionHeader::ReadData(std::ifstream& ifs)
{
...
/******************************************************************************/
void SpartanupPartitionHeader::Build(BootImage& bi, Binary& cache)
{
    if (section != NULL)
    {
...
/******************************************************************************/
void SpartanupPartitionHeader::Link(BootImage &bi, PartitionHeader* next_part_hdr)
{
...
/******************************************************************************/
void SpartanupPartitionHeader::SetPartitionSecureHdrIv(uint8_t* iv)
{
    if (iv == NULL)
    {
...
/******************************************************************************/
void SpartanupPartitionHeader::SetPartitionKeySrc(KeySource::Type keyType, BifOptions* bifOptions)
{
    kekIvMust = false;
    switch (keyType)
    {
...
/******************************************************************************/
void SpartanupPartitionHeader::SetEncryptedPartitionLength(uint32_t len)
{
    pHTable->encryptedPartitionLength = len / sizeof(uint32_t);
}

/******************************************************************************/
void SpartanupPartitionHeader::SetUnencryptedPartitionLength(uint32_t len)
{
    pHTable->unencryptedPartitionLength = len / sizeof(uint32_t);
}

/******************************************************************************/
void SpartanupPartitionHeader::SetTotalPartitionLength(uint32_t len)
{
...
/******************************************************************************/
void SpartanupPartitionHeader::SetNextPartitionHeaderOffset(uint32_t addr)
{
	pHTable->nextPartitionHeaderOffset = addr / sizeof(uint32_t);
}

/******************************************************************************/
void SpartanupPartitionHeader::SetExecAddress(uint64_t addr)
{
...
/******************************************************************************/
void SpartanupPartitionHeader::SetLoadAddress(uint64_t addr, bool versalNetSeries)
{
...
/******************************************************************************/
void SpartanupPartitionHeader::SetPartitionWordOffset(uint32_t addr)
{
    if (presigned)
    {
...
/******************************************************************************/
void SpartanupPartitionHeader::SetHashBlockWordOffset(uint32_t addr)
{
    pHTable->hashBlockWordOffset = addr / sizeof(uint32_t);
}

/******************************************************************************/
void SpartanupPartitionHeader::SetHashBlockLength(uint32_t length)
{
    pHTable->hashBlockLength1 = length;
}

/******************************************************************************/
void SpartanupPartitionHeader::SetPartitionAttributes(void)
{
    ...
    pHTable->partitionAttributes = (trustzone << supphtTrustzoneShift) |
                                   (exceptionLevel << supphtExceptionLevelShift) |
                                   (execState << supphtExecStateShift) |
                                   (destCpu << supphtDestCpuShift) |
                                   (checksumType << supphtChecksumTypeShift) |
                                   (pufHdLoc << supphtPufHDLocationShift) |
                                   (ownerType << supphtPartitionOwnerShift) |
                                   (endian << supphtEndiannessShift) |
                                   (partitionType << supphtPartitionTypeShift) |
                                   (hivec << supphtHivecShift) |
                                   (dpaCM << supphtDpaCMShift) |
                                   (cluster << supphtClusterShift) |
                                   (lockstep << supphtlockStepShift) |
                                   (tcmBoot << supphtTcmBootShift);
}

/******************************************************************************/
void SpartanupPartitionHeader::SetSectionCount(uint32_t cnt)
{
    pHTable->dataSectionCount = cnt;
}

/******************************************************************************/
void SpartanupPartitionHeader::SetChecksumOffset(void)
{
...
/******************************************************************************/
void SpartanupPartitionHeader::SetAuthCertificateOffset(void)
{
...
/******************************************************************************/
void SpartanupPartitionHeader::SetReserved(void)
{
...
/******************************************************************************/
void SpartanupPartitionHeader::SetPartitionId()
{
    pHTable->puid = partitionUid;
}

/******************************************************************************/
void SpartanupPartitionHeader::SetChecksum(void)
{
    pHTable->pHChecksum = ComputeWordChecksum(pHTable, sizeof(SpartanupPartitionHeaderTableStructure) - sizeof(uint32_t));
}
...
/******************************************************************************/
uint32_t SpartanupPartitionHeader::GetPartitionHeaderSize(void)
{
    return sizeof(SpartanupPartitionHeaderTableStructure);
}

/******************************************************************************/
uint32_t SpartanupPartitionHeader::GetSectionCount(void)
{
    return pHTable->dataSectionCount;
}

/******************************************************************************/
uint32_t SpartanupPartitionHeader::GetChecksum(void)
{
    return pHTable->pHChecksum;
}

/******************************************************************************/
uint64_t SpartanupPartitionHeader::GetExecAddress(void)
{
    return pHTable->destinationExecAddress;
}

/******************************************************************************/
uint64_t SpartanupPartitionHeader::GetLoadAddress(void)
{
    return pHTable->destinationLoadAddress;
}

/******************************************************************************/
uint32_t SpartanupPartitionHeader::GetEncryptedPartitionLength(void)
{
    return pHTable->encryptedPartitionLength * sizeof(uint32_t);
}

/******************************************************************************/
uint32_t SpartanupPartitionHeader::GetUnencryptedPartitionLength(void)
{
    return pHTable->unencryptedPartitionLength * sizeof(uint32_t);
}

/******************************************************************************/
uint32_t SpartanupPartitionHeader::GetTotalPartitionLength(void)
{
    return pHTable->totalPartitionLength * sizeof(uint32_t);
}

/******************************************************************************/
uint8_t SpartanupPartitionHeader::GetChecksumType(void)
{
    return ((pHTable->partitionAttributes >> supphtChecksumTypeShift) & supphtChecksumTypeMask);
}

/******************************************************************************/
uint8_t SpartanupPartitionHeader::GetOwnerType(void)
{
    return ((pHTable->partitionAttributes >> supphtPartitionOwnerShift) & supphtPartitionOwnerMask);
}

/******************************************************************************/
uint8_t SpartanupPartitionHeader::GetDestinationCpu(void)
{
    return ((pHTable->partitionAttributes >> supphtDestCpuShift) & supphtDestCpuMask);
}

/******************************************************************************/
uint8_t SpartanupPartitionHeader::GetDestinationCluster(void)
{
    return ((pHTable->partitionAttributes >> supphtClusterShift) & supphtClusterMask);
}

/******************************************************************************/
uint8_t SpartanupPartitionHeader::GetProcessorExecState(void)
{
    return ((pHTable->partitionAttributes >> supphtExecStateShift) & supphtExecStateMask);
}

/******************************************************************************/
uint8_t SpartanupPartitionHeader::GetElfEndianess(void)
{
    if ((pHTable->partitionAttributes >> supphtEndiannessShift) & supphtEndiannessMask)
    {
...
/******************************************************************************/
uint8_t SpartanupPartitionHeader::GetExceptionLevel(void)
{
    return ((pHTable->partitionAttributes >> supphtExceptionLevelShift) & supphtExceptionLevelMask);
}

/******************************************************************************/
uint8_t SpartanupPartitionHeader::GetTrustZone(void)
{
    return ((pHTable->partitionAttributes >> supphtTrustzoneShift) & supphtTrustzoneMask);
}

/******************************************************************************/
uint8_t SpartanupPartitionHeader::GetHivec(void)
{
    return ((pHTable->partitionAttributes >> supphtHivecShift) & supphtHivecMask);
}

/******************************************************************************/
uint32_t SpartanupPartitionHeader::GetAuthCertificateOffset(void)
{
    return (pHTable->authCertificateOffset * sizeof(uint32_t));
}

/******************************************************************************/
uint32_t SpartanupPartitionHeader::GetPartitionUid(void)
{
    return pHTable->puid;
}

/******************************************************************************/
uint32_t SpartanupPartitionHeader::GetPartitionWordOffset(void)
{
    return (pHTable->partitionWordOffset * sizeof(uint32_t));
}

/******************************************************************************/
PartitionType::Type SpartanupPartitionHeader::GetPartitionType(void)
{
    return (PartitionType::Type)((pHTable->partitionAttributes >> supphtPartitionTypeShift) & supphtPartitionTypeMask);
}

/******************************************************************************/
DpaCM::Type SpartanupPartitionHeader::GetDpaCMFlag(void)
{
    return (DpaCM::Type)((pHTable->partitionAttributes >> supphtDpaCMShift) & supphtDpaCMMask);
}

/******************************************************************************/
PufHdLoc::Type SpartanupPartitionHeader::GetPufHdLocation(void)
{
    return (PufHdLoc::Type)((pHTable->partitionAttributes >> supphtPufHDLocationShift) & supphtPufHDLocationMask);
}

/******************************************************************************/
Lockstep::Type SpartanupPartitionHeader::GetLockStepFlag (void)
{
    return (Lockstep::Type)((pHTable->partitionAttributes >> supphtlockStepShift) & supphtlockStepMask);
}

/******************************************************************************/
uint64_t SpartanupPartitionHeader::GetLQspiExecAddrForXip(uint64_t execAddr)
{
...
/******************************************************************************/
uint32_t SpartanupPartitionHeader::GetPartitionPadSize64bBoundary(Section* sec)
{
    return ((64 - (sec->Length & 63)) & 63);
}

/******************************************************************************/
void SpartanupPartitionHeader::SetPartitionRevokeId(uint32_t id)
{
    pHTable->partitionRevokeId = id;
}

/******************************************************************************/
KeySource::Type SpartanupPartitionHeader::GetPartitionKeySource(void)
{
    return (KeySource::Type)pHTable->partitionKeySource;
}

/******************************************************************************/
uint8_t SpartanupPartitionHeader::GetEncryptFlag(void)
{
...
/******************************************************************************/
uint8_t SpartanupPartitionHeader::GetAuthCertFlag(void)
{
...
/******************************************************************************/
TcmBoot::Type SpartanupPartitionHeader::GetTcmBootFlag(void)
{
    return (TcmBoot::Type)((pHTable->partitionAttributes >> supphtTcmBootShift) & supphtTcmBootMask);
}

/******************************************************************************/
void SpartanupPartitionHeaderTable::Build(BootImage & bi, Binary & cache)
{
...
#if 0
/******************************************************************************/
void SpartanupPartitionHeaderTable::UpdateAtfHandoffParams(BootImage & bi)
{
...
#endif
/******************************************************************************/
void SpartanupPartitionHeaderTable::Link(BootImage & bi)
{
    uint32_t numPart = 0;
    ...
    if (bi.imageHeaderTable->hashBlockSection != nullptr)
    {
        ...
        else
        {
            ...
            for (std::list<SubSysImageHeader*>::iterator image = bi.subSysImageList.begin(); image != bi.subSysImageList.end(); image++)
            {
                ...
                size += sizeof(SpartanupImageHeaderStructure);
            }
