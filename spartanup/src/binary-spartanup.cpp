...
*/
/******************************************************************************/
SpartanupBinary::~SpartanupBinary()
{
...
/******************************************************************************/
void SpartanupBinary::StackAndAlign(Options& options)
{
    std::string logmsg;
    ...
    for (SectionList::iterator i = Sections.begin(); i != Sections.end(); i++)
    {
        ...
        if ((!StringUtils::GetExtension(sec.Name).compare(".sha384") || !StringUtils::GetExtension(sec.Name).compare(".sha256") || !StringUtils::GetExtension(sec.Name).compare(".shake256")) && sec.Name.find(StringUtils::BaseName(options.GetFsblFilename())) != std::string::npos)
        {
            options.totalHeadersSize = runningAddress;
            options.allHeaderSize = options.totalHeadersSize - options.bootheaderSize;
            options.bootloaderSize = runningAddress;
    ...
    {
        Section& section(**i);
        if (!section.Name.compare(acBootloader_sha256) || !section.Name.compare(acBootloader_sha384) || !section.Name.compare(acBootloader_shake256))
        {
