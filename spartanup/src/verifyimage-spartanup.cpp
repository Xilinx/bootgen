...
/*******************************************************************************/
void SpartanupReadImage::VerifyAuthentication(bool verifyImageOption)
{
    ReadBinaryFile();
...
/******************************************************************************************************************************/
bool SpartanupReadImage::VerifyECDSASignature(bool nist, uint8_t * data, size_t dataLength, ACKeyECDSA *eckey, uint8_t* signature)
{
    uint8_t shaHash[SHA3_LENGTH_BYTES] = { 0 };
    Spartanupcrypto_hash(shaHash, data, dataLength, nist);
...
/******************************************************************************************************/
bool SpartanupReadImage::VerifyECDSAP521Signature(bool nist, uint8_t * data, size_t dataLength, ACKeyECDSAP521 *eckey, uint8_t* signature)
{
    uint8_t shaHash[SHA3_LENGTH_BYTES] = { 0 };
    Spartanupcrypto_hash(shaHash, data, dataLength, nist);
...
/*******************************************************************************/
bool SpartanupReadImage::VerifySignature(bool nist, uint8_t * data, size_t dataLength, ACKey4096Sha3Padding * acKey, uint8_t* signature)
{
    ...
    Spartanupcrypto_hash(maskPadHash, m1, 208, false);
...
/*******************************************************************************/
void SpartanupReadImage::VerifyHeaderTableSignature()
{
    Separator();
    ...
    else
    {
        if(smap_header_found)
        {
            offset = sizeof(SpartanupSmapWidthTable) + sizeof(SpartanupImageHeaderTableStructure);
        }
        else
        {
            offset = sizeof(SpartanupImageHeaderTableStructure);
        }
    }
    if (!(fseek(binFile, offset, SEEK_SET)))
    {
...
/*******************************************************************************/
void SpartanupReadImage::VerifySPKSignature(uint8_t* aC)
{
    size_t size = 0;
    bool signatureVerified = false;
...
/*******************************************************************************/
void SpartanupReadImage::VerifyPartitionSignature(void)
{
    size_t result;
    uint64_t offset = 0;
    bool signatureVerified = false;
    ...
    for (std::list<SpartanupImageHeaderStructure*>::iterator iH = iHs.begin(); iH != iHs.end(); iH++)
    {
