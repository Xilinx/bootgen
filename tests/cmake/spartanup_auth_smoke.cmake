...
execute_process(
    COMMAND "${BOOTGEN}" -arch spartanup -image "${TEST_FIXTURE_DIR}/keygen.bif" -generate_keys ecdsa
...
if(NOT KEYGEN_RESULT EQUAL 0 OR NOT EXISTS "${PRIMARY_KEY}" OR NOT EXISTS "${SECONDARY_KEY}")
    cleanup_test_keys()
    message(FATAL_ERROR "Spartan UltraScale+ ECDSA key generation failed (${KEYGEN_RESULT}):\n${KEYGEN_OUTPUT}\n${KEYGEN_ERROR}")
endif()
...
execute_process(
    COMMAND "${BOOTGEN}" -arch spartanup -image "${TEST_FIXTURE_DIR}/authenticated.bif" -o "${BOOT_IMAGE}" -w on
    WORKING_DIRECTORY "${TEST_FIXTURE_DIR}"
...
if(NOT CREATE_RESULT EQUAL 0)
    cleanup_test_keys()
    message(FATAL_ERROR "Authenticated Spartan UltraScale+ image creation failed (${CREATE_RESULT}):\n${CREATE_OUTPUT}\n${CREATE_ERROR}")
endif()
file(SIZE "${BOOT_IMAGE}" BOOT_IMAGE_SIZE)
if(BOOT_IMAGE_SIZE LESS 1024)
    cleanup_test_keys()
    message(FATAL_ERROR "Authenticated Spartan UltraScale+ image is unexpectedly small: ${BOOT_IMAGE_SIZE} bytes")
endif()
