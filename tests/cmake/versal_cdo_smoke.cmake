...
execute_process(
    COMMAND "${BOOTGEN}" -arch versal -image "${TEST_FIXTURE_DIR}/versal-cdo.bif" -o "${BOOT_IMAGE}" -w on
    WORKING_DIRECTORY "${TEST_FIXTURE_DIR}"
    RESULT_VARIABLE CREATE_RESULT
    OUTPUT_VARIABLE CREATE_OUTPUT
    ERROR_VARIABLE CREATE_ERROR)
if(NOT CREATE_RESULT EQUAL 0)
    message(FATAL_ERROR "Versal CDO image creation failed (${CREATE_RESULT}):\n${CREATE_OUTPUT}\n${CREATE_ERROR}")
endif()

file(SIZE "${BOOT_IMAGE}" BOOT_IMAGE_SIZE)
if(BOOT_IMAGE_SIZE LESS 512)
    message(FATAL_ERROR "Versal CDO smoke image is unexpectedly small: ${BOOT_IMAGE_SIZE} bytes")
endif()

execute_process(
    COMMAND "${BOOTGEN}" -arch versal -read "${BOOT_IMAGE}"
    RESULT_VARIABLE READ_RESULT
    OUTPUT_VARIABLE READ_OUTPUT
    ERROR_VARIABLE READ_ERROR)
if(NOT READ_RESULT EQUAL 0)
    message(FATAL_ERROR "Versal CDO image inspection failed (${READ_RESULT}):\n${READ_OUTPUT}\n${READ_ERROR}")
endif()

string(FIND "${READ_OUTPUT}" "partition_type[cdo]" CDO_OFFSET)
if(CDO_OFFSET EQUAL -1)
    message(FATAL_ERROR "Versal CDO smoke image does not contain a CDO partition:\n${READ_OUTPUT}")
endif()
...
execute_process(
    COMMAND "${BOOTGEN}" -arch versal -image "${TEST_FIXTURE_DIR}/versal-legacy-cdo.bif" -o "${OUTPUT_DIR}/legacy-BOOT.BIN" -w on
    WORKING_DIRECTORY "${TEST_FIXTURE_DIR}"
...
if(LEGACY_RESULT EQUAL 0 OR NOT LEGACY_RESULT MATCHES "^[1-9][0-9]*$")
    message(FATAL_ERROR "Versal accepted or crashed on an unsupported legacy CDO (result: ${LEGACY_RESULT}):\n${LEGACY_OUTPUT}\n${LEGACY_ERROR}")
endif()
