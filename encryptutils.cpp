...
uint32_t reverseDataBits(uint32_t value)
{
    ...
    // Keep reversing bits as long as there bits left.
    int valueBitLength = 32;
    while (valueBitLength > 0)
    {
        valueBitLength -= BITS_IN_BYTE;
        reversedValue |= (reverse8BitValueTable[value & 0xFF] << valueBitLength);
        value >>= BITS_IN_BYTE;
    }
