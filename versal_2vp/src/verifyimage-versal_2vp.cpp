...
/*******************************************************************************/
void Versal_2vpReadImage::VerifyAuthentication(bool verifyImageOption)
{
    ReadBinaryFile();
...
/******************************************************************************************************************************/
bool Versal_2vpReadImage::VerifyECDSASignature(bool nist, uint8_t * data, size_t dataLength, ACKeyECDSA *eckey, uint8_t* signature)
{
    uint8_t shaHash[SHA3_LENGTH_BYTES] = { 0 };
    Versal_2vpcrypto_hash(shaHash, data, dataLength, nist);
...
/******************************************************************************************************/
bool Versal_2vpReadImage::VerifyECDSAP521Signature(bool nist, uint8_t * data, size_t dataLength, ACKeyECDSAP521 *eckey, uint8_t* signature)
{
    uint8_t shaHash[SHA3_LENGTH_BYTES] = { 0 };
    Versal_2vpcrypto_hash(shaHash, data, dataLength, nist);
...
/*******************************************************************************/
bool Versal_2vpReadImage::VerifySignature(bool nist, uint8_t * data, size_t dataLength, ACKey4096Sha3Padding * acKey, uint8_t* signature)
{
    ...
    uint8_t masK[463] = { 0 };
    RSA4096Sha3PaddingHBAuthenticationAlgorithm_versal_2vp* s = NULL;
    ...
    Versal_2vpcrypto_hash(maskPadHash, m1, 208, false);
...
/*******************************************************************************/
void Versal_2vpReadImage::VerifyHeaderTableSignature()
{
    Separator();
    ...
    uint32_t iHTLength = sizeof(Versal_2vpImageHeaderTableStructure);
    ...
    if(bH != NULL)
    {
        offset = bH->imageHeaderByteOffset + sizeof(Versal_2vpImageHeaderTableStructure);
    }
    else
    {
        if(smap_header_found)
        {
            offset = sizeof(Versal_2vpSmapWidthTable) + sizeof(Versal_2vpImageHeaderTableStructure);
        }
        else
        {
            offset = sizeof(Versal_2vpImageHeaderTableStructure);
        }
    }
    if (!(fseek(binFile, offset, SEEK_SET)))
    {
...
/*******************************************************************************/
void Versal_2vpReadImage::VerifySPKSignature(uint8_t* aC)
{
    bool signatureVerified = false;
    ...
    if ((temp & 0xF3) == 0x02)
    {
        /* versal_2vp ECDSA P-384 hash-block AC: the PPK signs the contiguous block
         * [spkheader(32) + acSpk(96)] = AC[ECDSA_AC_SPK_HDR_OFFSET .. +128].
         * The hash is computed with the NIST/Versal sha (nist=true), matching
         * GenerateSPKHash()'s CalculateVersalHash(true,...) on generation side. */
        size_t size = sizeof(HeaderAuthCert_versal_2vp) + (2 * EC_P384_KEY_LENGTH);
        signatureVerified = VerifyECDSASignature(true, aC + ECDSA_AC_SPK_HDR_OFFSET, size,
                                                 (ACKeyECDSA *)(aC + ECDSA_AC_PPK_KEY_OFFSET),
                                                 aC + ECDSA_AC_SPK_SIGN_OFFSET);
    }
    else if ((temp & 0xF3) == 0x22)
    {
        size_t size = sizeof(uint32_t) + sizeof(uint32_t) + VERSAL_ACKEY_STRUCT_SIZE + AC_SPK_SHA3PAD_SIZE;
...
/*******************************************************************************/
void Versal_2vpReadImage::VerifyPartitionSignature(void)
{
    size_t result;
    uint64_t offset = 0;
    bool signatureVerified = false;
    ...
    std::list<Versal_2vpPartitionHeaderTableStructure*>::iterator partitionHdr = pHTs.begin();
    for (std::list<Versal_2vpImageHeaderStructure*>::iterator iH = iHs.begin(); iH != iHs.end(); iH++)
    {
        for ( uint32_t cnt_index = 0; cnt_index < (*iH)->dataSectionCount; cnt_index++,partitionHdr++,auth_cert++)
        {
            if ((*partitionHdr)->authCertificateOffset != 0)
            {
                ...
                if (isItBootloader)
                {
                    uint32_t bHLength = sizeof(Versal_2vpBootHeaderStructure) - sizeof(Versal_2vpSmapWidthTable);
                ...
                else
                {
                    ...
                    if (signatureVerified)
                    {
                        ...
                        for (int i = 1; i < count; i++)
                        {
                            ...
                            Versal_2vpcrypto_hash(shaHash, dataBuffer, dataSize, true);
