...
/******************************************************************************/
void Versal_2vpPartition::CalculateChunkificationHash(uint8_t* shaHash, uint8_t* data, size_t inlen, bool padding)
{
    Versalcrypto_hash(shaHash, data, inlen, padding);
}

/******************************************************************************/
void Versal_2vpPartition::DumpPCRHashes(BootImage & bi)
{
    if (getenv("BOOTGEN_GENERATE_PCR_HASHES") != NULL)
    {
        ...
        if (header->partition->section->isBootloader)
        {
            ...
            bi.hash->CalculateVersalHash(true, header->partition->section->Data.get(), length, sha_hash);
            ...
            {
                memset(sha_hash, 0, chunkificationHashLength);
                bi.hash->CalculateVersalHash(true, header->partition->section->Data.get() + length, header->imageHeader->GetTotalPmcFwSizeIh(), sha_hash);
        ...
        {
            bi.hash->CalculateVersalHash(true, header->partition->section->Data.get(), header->partition->section->Length, sha_hash);
        }
        delete sha_hash;
    }
}
...
/******************************************************************************/
Versal_2vpPartition::Versal_2vpPartition(PartitionHeader* header0, Section* section0)
...
/******************************************************************************/
Versal_2vpPartition::Versal_2vpPartition(PartitionHeader* header0, const uint8_t* data, Binary::Length_t length)
...
/******************************************************************************/
size_t Versal_2vpPartition::GetTotalDataChunks(Binary::Length_t partitionSize, std::vector<uint32_t>& dataChunks, bool encryptionFlag)
{
...
/******************************************************************************/
size_t Versal_2vpPartition::GetBootloaderTotalDataChunks(Binary::Length_t partitionSize, std::vector<uint32_t>& dataChunks, bool encryptionFlag)
{
...
/******************************************************************************/
void Versal_2vpPartition::CopyPlmPmcHashesToBootloaderHashBlock(Section* section, BootImage& bi)
{
    Versal_2vpBootImage& vbi = static_cast<Versal_2vpBootImage&>(bi);
    if (!section || !vbi.bootloaderHashBlockSection) return;

    uint8_t* hashBlockData = vbi.bootloaderHashBlockSection->Data.get();
    uint32_t offset = 0;
    size_t chunkificationHashLength = this->chunkificationHashLength;

    // --- Leave space for BH hash (index 0) ---
    offset += sizeof(uint32_t) + chunkificationHashLength;

    // --- PLM hash (index 1) ---
    if (header->imageHeader->GetTotalFsblFwSizeIh() != 0) {

        uint32_t plmIndex = HASH_BLOCK_PLM_HASH_INDEX;
        memcpy(hashBlockData + offset, &plmIndex, sizeof(uint32_t));
        offset += sizeof(uint32_t);

        // Correct PLM hash offset: skip BH index, BH hash, PLM index
        size_t plm_hash_offset = 2 * sizeof(uint32_t) + chunkificationHashLength;
        memcpy(hashBlockData + offset, section->Data.get() + plm_hash_offset, chunkificationHashLength);
        offset += chunkificationHashLength;
    }
    else
    {
        offset += sizeof(uint32_t); //skipping PLM hash index
        offset += chunkificationHashLength; //skipping PLM hash length
    }

    // --- PMC hash (index 2, if present) ---
    if (header->imageHeader->GetTotalPmcFwSizeIh() != 0) {
        uint32_t pmcIndex = HASH_BLOCK_PMCDATA_HASH_INDEX;
        memcpy(hashBlockData + offset, &pmcIndex, sizeof(uint32_t));
        offset += sizeof(uint32_t);

        // Correct PMC hash offset: skip BH index, BH hash, PLM index, PLM hash, PMC index
        size_t pmc_hash_offset = 3 * sizeof(uint32_t) + 2 * chunkificationHashLength;
        memcpy(hashBlockData + offset, section->Data.get() + pmc_hash_offset, chunkificationHashLength);
        offset += chunkificationHashLength;
    }
    else 
    {
        offset += sizeof(uint32_t); //skipping PMC hash index
        offset += chunkificationHashLength; //skipping PMC hash length
    }

    // --- Leave space for metaheader hash (index 3) ---
    offset += sizeof(uint32_t) + chunkificationHashLength;

    // Optionally zero any remaining bytes in the hashblock
    if (offset < vbi.bootloaderHashBlockSection->Length) {
        memset(hashBlockData + offset, 0, vbi.bootloaderHashBlockSection->Length - offset);
    }
    // Print the final hash block data for verification
    LOG_INFO("Final bootloader hash block data:");
    LOG_DUMP_BYTES(hashBlockData, vbi.bootloaderHashBlockSection->Length);
}

/******************************************************************************/
void Versal_2vpPartition::ChunkifyAndHash(Section* section, bool encryptionFlag)
{
...
/******************************************************************************/
void Versal_2vpPartition::Build(BootImage& bi, Binary& cache)
{
    ...
    else
    {
        ...
        //if ((imageHeader.GetChecksumContext()->Type() == Checksum::SHA3) || (currentAuthCtx->authAlgorithm->Type() != Authentication::None))
        {
            ...
            if (imageHeader.IsBootloader())
            {
                // FIX start:
                if (header->imageHeader->GetAuthenticationType() != Authentication::None ||
                    (header->imageHeader->GetAuthenticationType() == Authentication::None && encryptCtx->Type() != Encryption::None))
                {
                    Versal_2vpBootImage& vbi = static_cast<Versal_2vpBootImage&>(bi);

                    // Create the new hash block section
                    std::string hashBlockName = section->Name + ".HashBlock";
                    uint32_t hashBlockSize = BH_HASH_BLOCK_BYTES_TELLURIDE; 

                    Section* bootloaderHashBlock = new Section(hashBlockName, hashBlockSize);
                    memset(bootloaderHashBlock->Data.get(), 0, hashBlockSize);

                    // Store pointer for later population (checksum, authentication, etc.)
                    vbi.bootloaderHashBlockSection = bootloaderHashBlock;

                    // Insert after metaheader hashblock (if you need strict ordering, insert at correct index)
                    cache.Sections.push_back(std::unique_ptr<Section>(bootloaderHashBlock));

                    LOG_INFO("Created new section for bootloader hash block: %s", hashBlockName.c_str());
                }
                // ==== Fix End here
                if (header->imageHeader->GetTotalPmcFwSizeIh() != 0)
                {
            ...
            else
            {
                ...
                if (bi.partitionHeaderList.front()->IsBootloader() == false)
                {
                ...
                if (dataChunksCount != 1)
                {
                    ...
                    CalculateChunkificationHash(shaHash, section->Data.get(), firstChunkSize + chunkificationHashLength, true);
                }
                if (dataChunksCount == 1)
                {
                    CalculateChunkificationHash(shaHash, section->Data.get(), section->Length, true);
                }

                memcpy(bi.hashTable.back().second.get(), shaHash, chunkificationHashLength);
        ...
        /* Modified : Push the bootloader partition section before its Authentication Certificate */
     /*   if (imageHeader.IsBootloader())
        {
            if (section != NULL)
            {
         ...
         * - For bootloader (PLM/PMC): Create AC for Hash Block 0
         * - For PLM loadable partitions: Do NOT create individual ACs
         *   Their hashes are added to bi.hashTable and consolidated into Hash Block 1.
         *   The Meta Header AC will sign Hash Block 1.
         */
           if (!imageHeader.IsBootloader() && currentAuthCtx->authAlgorithm->Type() != Authentication::None)
           {
                ...
                if (count > 0)
                {
                    /* Hash entries for the whole group (owner + sharers). */
                    hashBlockSectionLength = (HASH_BLOCK_INDEX_BYTES + bi.hash->GetHashLength()) * count;
                    hashBlockSectionLength += PADDING_16B(hashBlockSectionLength);

                    hashBlockSection = new Section("HashBlock", hashBlockSectionLength);
                    memset(hashBlockSection->Data.get(), 0, hashBlockSection->Length);
       ...
       {
           cache.Sections.push_back(std::unique_ptr<Section>(hashBlockSection));
       }

       /* Create AC for: the bootloader (HB0) OR a non-bootloader owner (its HBn). */
       if (currentAuthCtx->authAlgorithm->Type() != Authentication::None &&
           (imageHeader.IsBootloader() || hashBlockSection != NULL))
       {
           AuthenticationCertificate* tempacs;
           tempacs = new Versal_2vpAuthenticationCertificate(currentAuthCtx);
    ...
    if (imageHeader.IsBootloader() == true)
    {
        if (encryptCtx->Type() != Encryption::None && currentAuthCtx->authAlgorithm->Type() == Authentication::None)
        {
            ...
            Versal_2vpBootHeaderStructure* bh = (Versal_2vpBootHeaderStructure*)bi.bootHeader->section->Data.get();
            ...
            LOG_DUMP_BYTES(tmpBh, bi.bootHeader->GetBootHeaderSize() - sizeof(Versal_2vpSmapWidthTable));
            /* Copy BH Hash */
            memcpy(header->partition->section->Data.get() + HASH_BLOCK_INDEX_BYTES, sha_hash, chunkificationHashLength);
    ...
    if (section != NULL)
    {
        if (imageHeader.IsBootloader()) 
        {
            bool is_auth = imageHeader.GetAuthenticationType() != Authentication::None;
            bool is_enc  = encryptCtx->Type() != Encryption::None;

            // Only run unclubbing for secure flows (auth or enc)
            if (is_auth || is_enc) {
                CopyPlmPmcHashesToBootloaderHashBlock(section, bi);
              
                size_t plm_pmc_size = imageHeader.GetTotalFsblFwSizeIh() + imageHeader.GetTotalPmcFwSizeIh();
                LOG_INFO("PARTITION BUILD: Expected PLM+PMC size: 0x%X", plm_pmc_size);

                // Only unclub if extra metadata is present
                if (section->Length > plm_pmc_size) {
                    LOG_INFO("PARTITION BUILD: Unclubbing PLM section - removing metadata");
                    // Always copy only the last plm_pmc_size bytes (PLM+PMC)
                    uint8_t* plm_pmc_ptr = section->Data.get() + (section->Length - plm_pmc_size);
                    uint8_t* plm_pmc_data = new uint8_t[plm_pmc_size];
                    memcpy(plm_pmc_data, plm_pmc_ptr, plm_pmc_size);

                    section->Data.reset(plm_pmc_data);
                    section->Length = plm_pmc_size;
                } else {
                    LOG_INFO("PARTITION BUILD: No unclubbing needed - section already contains only PLM+PMC");
                }
            }
            size_t plm_size = imageHeader.GetTotalFsblFwSizeIh();
            size_t pmc_size = imageHeader.GetTotalPmcFwSizeIh();

            LOG_TRACE("PLM Content (first 512 bytes) post unclub:");
            LOG_DUMP_BYTES(section->Data.get(), (plm_size > 512) ? 512 : plm_size);

            if (pmc_size > 0) {
                LOG_TRACE("PMC Content (first 512 bytes) post unclub:");
                LOG_DUMP_BYTES(section->Data.get() + plm_size, (pmc_size > 512) ? 512 : pmc_size);
            }
        }  // For non-secure, do nothing (PLM+PMC already)
        cache.Sections.push_back(std::unique_ptr<Section>(section));
    }
}

/******************************************************************************/
void Versal_2vpPartition::Link(BootImage &bi)
{
    if (hashBlockSection != NULL && bi.hashNumMap.size() != 0)
    {
        ...
        {
            if (bi.hashNumMap[k].second != adjustedPartNum)
                continue;
            uint32_t partNum = bi.hashNumMap[k].first;
            for (size_t i = 1; i < bi.hashTable.size(); i++)
            {
                if (bi.hashTable[i].first == partNum)
                {
                    memcpy(hashBlockSection->Data.get() + (slot * entrySize),
                           &partNum, HASH_BLOCK_INDEX_BYTES);
                    memcpy(hashBlockSection->Data.get() + HASH_BLOCK_INDEX_BYTES + (slot * entrySize),
    ...
    for (std::list<AuthenticationCertificate*>::iterator acs = header->ac.begin(); acs != header->ac.end(); acs++)
    {
        if ((*acs))
        {
            /* Multi-SPK owner: sign this owner's hash block (signature lands
               in the AC tail). Bootloader/legacy: sign the partition data. */
            if (hashBlockSection != NULL)
                (*acs)->Link(bi, hashBlockSection);
            else
