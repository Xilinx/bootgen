...
-------------------------------------------------------------------------------
*/
#include "encryption-versal_2vp.h"
...
/******************************************************************************/
Versal_2vpEncryptionContext::Versal_2vpEncryptionContext()
    : isBootloader(false)
    , aesSeedexits(false)
    , isPmcData(false)
...
/******************************************************************************/
Versal_2vpEncryptionContext::Versal_2vpEncryptionContext(const EncryptionContext* other)
    : isBootloader(false)
    , aesSeedexits(false)
    , isPmcData(false)
    , isAuthenticated(false)
...
/******************************************************************************/
void Versal_2vpEncryptionContext::SetAesKey(const uint8_t* key)
{
...
/******************************************************************************/
void Versal_2vpEncryptionContext::SetAesSeedString(const std::string& key)
{
    uint8_t  hexData[256];
...
/******************************************************************************/
void Versal_2vpEncryptionContext::GenerateAesKey(void)
{
...
/******************************************************************************/
void Versal_2vpEncryptionContext::GetEncryptionKeys(Options& options, uint8_t* aesKey,
...
/******************************************************************************/
void Versal_2vpEncryptionContext::WriteEncryptionKeyFile(const std::string & baseFileName, 
                                                       bool useOptionalKey, uint32_t blocks)
{
    ...
    for (x = 0; x<blocks; x++) 
    {
        ...
        if (x == 0 && GetAesKey() != NULL) 
        {
            ...
            {
                keyFile << std::uppercase << std::hex << std::setfill('0') << std::setw(8) << aesKey.get()[index];
            }
        }
        else 
        {
...
/******************************************************************************/
void Versal_2vpEncryptionContext::ReadEncryptionKeyFile(const std::string& inputFileName)
{
...
/******************************************************************************/
void Versal_2vpEncryptionContext::PackNextEncryptionKey(uint8_t* aesKeyNext, int aeskeyPtr)
{
...
/******************************************************************************/
void Versal_2vpEncryptionContext::PackNextIv(uint8_t* aesIVNext, int aesIvPtr)
{
...
/******************************************************************************/
void Versal_2vpEncryptionContext::GetNextKey(uint8_t * keyNext, int ptr)
{
    PackNextEncryptionKey(keyNext, ptr);
}

/******************************************************************************/
void Versal_2vpEncryptionContext::GetNextIv(uint8_t * keyNext, int ptr)
{
    PackNextIv(keyNext, ptr);
}

/******************************************************************************/
void Versal_2vpEncryptionContext::SetAesSeed(const uint8_t* key)
{
...
/******************************************************************************/
const uint32_t* Versal_2vpEncryptionContext::GetAesSeed(void)
{
    return aesSeed.get();
}

/******************************************************************************/
const uint32_t* Versal_2vpEncryptionContext::GetAesKey(void)
{
    return aesKey.get();
}

/******************************************************************************/
void Versal_2vpEncryptionContext::SetIv(const uint8_t* iv)
{
...
/******************************************************************************/
void Versal_2vpEncryptionContext::SetIvString(const std::string& IV)
{
    uint8_t  hexData[256];
...
/******************************************************************************/
void Versal_2vpEncryptionContext::GenerateIv(void)
{
...
/******************************************************************************/
const uint32_t* Versal_2vpEncryptionContext::GetIv(void)
{
    return aesIv.get();
}

/******************************************************************************/
void Versal_2vpEncryptionContext::GenerateRemainingKeys(Options& options)
{
    uint32_t blocks, x = 0;
    ...
    {
        aesSeed = std::make_unique<uint32_t[]>(WORDS_PER_AES_KEY);
        memset(aesSeed.get(), 0, WORDS_PER_AES_KEY * sizeof(uint32_t));
        GenerateAesSeed();
    }
    ...
    if (GetFixedInputData() == NULL)
    {
        ...
        memset(fixedInputData.get(), 0, WORDS_PER_FID * sizeof(uint32_t));
    ...
    for (x = 0; x < blocks; x++)
    {
        memcpy(aesKeyNext, &outBufKDF.get()[x * 11], AES_GCM_KEY_SZ);
        ...
        memcpy(aesIvNext, &outBufKDF.get()[(x * 11) + WORDS_PER_AES_KEY], AES_GCM_IV_SZ);
...
/******************************************************************************/
void Versal_2vpEncryptionContext::AesGcm256HashBlockEncrypt(Options& options, uint8_t *aad, uint32_t aad_len, uint8_t* outBuf, uint8_t ivIncrement)
{
...
/******************************************************************************/
void Versal_2vpEncryptionContext::ChunkifyAndEncrypt(Options& options, const uint8_t *inBuf, uint32_t inLen,
...
/******************************************************************************/
void Versal_2vpEncryptionContext::WarnforDPACMImpactonBootTime(bool dpacmKeyRollingEnable, bool dpacmMaskEnable)
{
...
/******************************************************************************/
void Versal_2vpEncryptionContext::Process(BootImage& bi, PartitionHeader* partHdr)
{
    ...
    auto keyCopy = std::make_unique<uint32_t[]>(AES_GCM_KEY_SZ / 4);
    memcpy(keyCopy.get(), aesKey.get(), AES_GCM_KEY_SZ);
    bi.aesKeyandKeySrc.push_back(std::make_pair(partHdr->imageHeader->GetEncryptionKeySrc(), std::move(keyCopy)));
    ...
    if (isBootloader && partHdr->imageHeader->GetPmcFwSizeIh() != 0)
    {
        ...
        memcpy(partHdr->partition->section->Data.get(), encrFsblDataBuffer, encrFsblByteLength);
        memcpy(partHdr->partition->section->Data.get() + encrFsblByteLength, encrPmcDataBuffer, encrPmcByteLength);
    ...
    else
    {
        ...
        memcpy(partHdr->partition->section->Data.get(), encryptedDataBuffer, encryptedLength);
...
/******************************************************************************/
void Versal_2vpEncryptionContext::Process(BootImage& bi)
{
    size_t size = bi.imageHeaderTable->metaHeaderLength;
    uint8_t* dataBuffer = new uint8_t[size];

    uint32_t offset = 0;
    for (auto i = bi.headers.begin(); i != bi.headers.end(); i++)
    {
        Section* section = *i;
        memcpy(dataBuffer + offset, section->Data.get(), section->Length);
    ...
    if (bi.options.bifOptions->metaHdrAttributes.encrKeyFile != "")
    {
        ...
        auto keyCopy = std::make_unique<uint32_t[]>(AES_GCM_KEY_SZ / 4);
        memcpy(keyCopy.get(), aesKey.get(), AES_GCM_KEY_SZ);
        bi.aesKeyandKeySrc.push_back(std::make_pair(bi.options.bifOptions->metaHdrAttributes.encrKeySource, std::move(keyCopy)));
        bi.bifOptions->CheckForBadKeyandKeySrcPair(bi.aesKeyandKeySrc, aesFilename);
    ...
    ChunkifyAndEncrypt(bi.options,
        ...
        sizeof(Versal_2vpImageHeaderTableStructure) + bi.imageHeaderTable->iht_optional_data_length,
        encryptedDataBuffer /* out*/,
        encryptedLength /* out */);
    ...
    memcpy(bi.encryptedHeaders->Data.get(), encryptedDataBuffer, encryptedLength);
...
/******************************************************************************/
void Versal_2vpEncryptionContext::AesGcm256Encrypt(unsigned char* gcm_pt, int pt_len,
...
/******************************************************************************/
void Versal_2vpEncryptionContext::AesGcm256Decrypt(unsigned char* gcm_pt, int& pt_len,
                                               unsigned char* gcm_key, unsigned char* gcm_iv,
                                               unsigned char* gcm_aad, int aad_len,
                                               unsigned char* gcm_ct, int ct_len,
                                               unsigned char* gcm_tag)
...
/******************************************************************************/
uint32_t Versal_2vpEncryptionContext::ConfigureEncryptionBlocksforPmcData(BootImage& bi, PartitionHeader* partHdr)
{
    uint32_t totalencrBlocks = 0;
    Binary::Length_t lastBlock = 0;
    std::vector<uint32_t> encrBlocks = partHdr->imageHeader->GetEncrBlocksList();
...
/******************************************************************************/
uint32_t Versal_2vpEncryptionContext::ConfigureEncryptionBlocksforPartition(BootImage& bi, PartitionHeader* partHdr)
{
    Options& options = bi.options;
    uint32_t totalencrBlocks = 0;
    Binary::Length_t lastBlock = 0;
...
/******************************************************************************/
void Versal_2vpEncryptionContext::ChunkifyAndProcess(BootImage& bi, PartitionHeader* partHdr)
{
    Options& options = bi.options;
    ...
    if (isBootloader && partHdr->imageHeader->GetPmcFwSizeIh() != 0)
    {
        ...
        if (bi.pmcDataAesFile != "")
        {
            ...
            if (tmpIv != NULL)
            {
                ...
                memcpy(options.secHdrIvPmcData.get(), bh->plmSecureHdrIv, BYTES_PER_IV);
            }
        }
        else
        {
