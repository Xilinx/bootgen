...
#ifndef _READIMAGE_VERSAL_2VP_H_
#define _READIMAGE_VERSAL_2VP_H_
...
*/
/******************************************************************************/
class Versal_2vpReadImage :public ReadImage
{
public:
    Versal_2vpReadImage(std::string filename) : ReadImage(filename)
    {
    ...
    Authentication::Type DetectAuthTypeFromAC(uint32_t acOffset);
    std::string GetPartitionType(uint32_t);
    std::string GetPartitionCore(uint32_t);

protected:
    std::string binFilename;
    Versal_2vpBootHeaderStructure* bH;
    Versal_2vpImageHeaderTableStructure* iHT;
    Versal_2vpImageHeaderStructure *iH;
    Versal_2vpPartitionHeaderTableStructure *pHT;
    std::list<Versal_2vpImageHeaderStructure*> iHs;
    std::list<Versal_2vpPartitionHeaderTableStructure*> pHTs;
    std::list<uint8_t*> aCs;
    std::list<uint32_t> acSizes;
    std::list<std::pair<uint8_t*, uint32_t>> Hashblock_record;  
    std::list<uint32_t> authtype;
    uint32_t currentAcSize;
