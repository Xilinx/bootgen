...
#ifndef _BOOTHEADER_VERSAL_2VP_H_
#define _BOOTHEADER_VERSAL_2VP_H_
...
#define PLM_BH_RESERVED                         23 /* #define PLM_BH_RESERVED_VERSAL_VERSALNET_TELLURIDE_LASSEN   23*/
...
/* Versal_2vp */
#define UDF_BH_VERSAL_2VP                        128 /* 512 Bytes */
#define ROM_BH_RESERVED_VERSAL_2VP               18 /* 72 bytes */
...
} Versal_2vpSmapWidthTable;




/* Versal_2vp BH Structure */
typedef struct
{
    uint32_t smapWords[SMAP_BUS_WIDTH];                           // (0x00)
    ...
    uint32_t pufRoSwapConfigVal;                                  // (0x74)
    uint32_t plmRevokeId;                                         // (0x78)
    uint32_t udfBH[UDF_BH_VERSAL_2VP];                             // (0x7C)
    uint32_t hashAlgo;                                            // (0x27C)  // Only hash algorithm field remains from AH
    uint32_t hashBlockLength1;                                    // (0x280)
    /* Reserved starts from 0x284, but added pufPDIIdentificationWord and pufHDLength to compensate Lassen BH Struct */
    uint32_t pufPDIIdentificationWord;                            // (0x284)
    uint32_t pufHDLength;                                          // (0x288)   
    uint32_t romReserved[ROM_BH_RESERVED_VERSAL_2VP];             // (0x28C)
    uint32_t imageHeaderByteOffset;                               // (0x2D4)
    uint32_t plmReserved[PLM_BH_RESERVED ];                        // (0x2D8)
    uint32_t reginit[MAX_REG_INIT_VERSAL];                        // (0x334)
    uint32_t puf[PUF_DATA_LENGTH / 4];                            // (0xB34)
    uint32_t headerChecksum;                                      // (0x113c)
                                                                  // (0x1140)
} Versal_2vpBootHeaderStructure;

#define BH_AC_HEADER_OFFSET_V2                     0x280
#define BH_TOTAL_PPK_SIZE1_OFFSET_V2               0x288
#define BH_ACTUAL_PPK_SIZE1_OFFSET_V2              0x28C
#define BH_TOTAL_SIGN_SIZE1_OFFSET_V2              0x290
#define BH_ACTUAL_SIGN_SIZE1_OFFSET_V2             0x294
...
*/
/******************************************************************************/
class Versal_2vpBootHeader : public BootHeader
{
public:
    Versal_2vpBootHeader(Arch::Type arch);
    Versal_2vpBootHeader(std::ifstream& src, Arch::Type arch);
    ~Versal_2vpBootHeader();
    ...
    void SetUdfBhTelluride(uint32_t file);
   ...
   // void SetSignatureSize(uint32_t actualSize, uint32_t totalSize);
    void SetHashAlgo(BootImage &bi, uint32_t value);
    ...
    uint32_t GetHashBlockLength(void);
    uint32_t GetAuthCertificateSize(PartitionHeader& bootloaderHdr);
    ...
    Arch::Type arch;
private:
    Versal_2vpBootHeaderStructure *bHTable;
    Versal_2vpSmapWidthTable *smapTable;
    bool kekIvMust;
