...
#ifndef _VERSAL_2VP_IMAGEHEADERTABLE_H_
#define _VERSAL_2VP_IMAGEHEADERTABLE_H_
...
typedef struct
{
    ...
    uint32_t    hashBlockLength;                    // 0x5C - Hash block length for Meta header (in words)
    uint32_t    hashBlockOffset;                    // 0x60 - Meta header hash block offset (in words)
    uint32_t    reserved[6];                        // 0x64-0x78 - Reserved (0x0)
    uint32_t    ihtChecksum;                        // 0x7C
} Versal_2vpImageHeaderTableStructure;
...
} Versal_2vpImageHeaderStructure;
...
class Versal_2vpImageHeaderTable : public ImageHeaderTable
{
public:
    Versal_2vpImageHeaderTable();
    Versal_2vpImageHeaderTable(std::ifstream& ifs);
    ~Versal_2vpImageHeaderTable();
    ...
    void SetMetaHdrAuthenticationFields(BootImage& bi);
...
private:
    Versal_2vpImageHeaderTableStructure *iHTable;
    uint8_t creatorId;
    uint32_t parentId;
    uint32_t pdiId;
    uint32_t idCode;
...
/******************************************************************************/
class Versal_2vpImageHeader : public ImageHeader
{
public:
    Versal_2vpImageHeader(std::string& filename);
    Versal_2vpImageHeader(uint8_t* data, uint64_t len);
    Versal_2vpImageHeader(std::ifstream& ifs, bool isBootloader);
    Versal_2vpImageHeader(std::ifstream& ifs, Versal_2vpImageHeaderStructure* importedIH, bool isBootloader, uint32_t index);
    ~Versal_2vpImageHeader();
...
private:
    ...
    Versal_2vpImageHeaderStructure *imageHeader;
...
/******************************************************************************/
class Versal_2vpSubSysImageHeader : public SubSysImageHeader
{
public:
    Versal_2vpSubSysImageHeader(ImageBifOptions * imgOptions);
    Versal_2vpSubSysImageHeader(std::ifstream & ifs);
...
protected:
    Versal_2vpImageHeaderStructure *versal_2vpSubSysImageHeaderTable;
};
#endif
