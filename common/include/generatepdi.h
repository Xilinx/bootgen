...
public:
    ...
    {
        partitionType = xtype;
        partitionBuffer = std::make_unique<uint8_t[]>(size);
        memcpy(partitionBuffer.get(), buffer, size);
        partitionSize = size;
        puid = id;
        switch (xtype)
        {
    ...
    {
        // partitionBuffer cleanup handled by unique_ptr
    }
...
protected:
    ...
    std::unique_ptr<uint8_t[]> partitionBuffer;
...
public:
    ...
    virtual ~PdiImage()  // Made virtual for proper polymorphic deletion
    {
    }
